...
"Embed management" refers to the ability for administrative users to associate multiple video embeds (e.g., from YouTube, Vimeo) with a single event. This feature will allow for a richer, more dynamic event page by incorporating pre-recorded videos, tributes, or other relevant media alongside the primary livestream.

## 2. Data Model Changes

To support embed management, we will introduce a new sub-collection to our Firestore data model.

-   **Sub-collection:** A new sub-collection named `embeds` will be added to each `event` document in the `memorials` collection.
...
This approach keeps the primary `event` document clean and allows for a scalable number of embeds per event.
...
We will also need a `GET` endpoint to list embeds for a given event, which can be part of the main event data fetching or a separate endpoint if needed for the admin UI. For simplicity, we can start by fetching them along with the event data for the admin portal.

## 4. UI/UX Plan

The user interface for managing embeds will be integrated into the existing `AdminPortal.svelte` component.

1.  **Access Point:** A new "Manage Embeds" button will be added to the actions column for each event listed in the admin portal table.
2.  **Modal Interface:** Clicking the "Manage Embeds" button will open a modal window.
3.  **Modal Content:** The modal will contain:
    *   A list of existing embeds associated with the selected event. Each item in the list will show the embed's title and have "Edit" and "Delete" buttons.
