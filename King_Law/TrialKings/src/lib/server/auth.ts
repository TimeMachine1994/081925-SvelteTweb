...
export async function getOrCreateUser(email: string): Promise<table.User> {
...
export async function createMagicLink(email: string): Promise<string> {
	...
	const magicLink: table.MagicLink = {
		id: generateId(),
		email: normalizedEmail,
		token,
...
export async function verifyMagicLink(token: string): Promise<{ email: string } | null> {
	const [link] = await db
		...
		.where(eq(table.magicLink.token, token));
	
	if (!link || link.used || Date.now() >= link.expiresAt.getTime()) {
		return null;
	}
	...
	await db
		...
		.set({ used: true })
		.where(eq(table.magicLink.id, link.id));
