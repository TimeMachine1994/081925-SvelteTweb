...
<div>
	<div class="flex justify-between items-center mb-6">
		...
		<button 
			...
			class="bg-gold hover:bg-gold-dark text-black font-semibold px-4 py-2 rounded-md transition-colors text-sm"
		>
			+ New Case
		</button>
	</div>
	...
	{#if activeTab === 'overview'}
				...
				<div class="flex items-start gap-3">
					...
					<div class="flex-1">
						<div class="flex justify-between items-center mb-3">
							<h3 class="font-semibold">New Client Registrations ({visibleNewClients.length})</h3>
							<button 
									...
									<div class="flex justify-between items-start mb-2">
										<div>
											...
											<div class="text-sm text-muted-foreground">
												{client.email}{#if client.phoneNumber} • {client.phoneNumber}{/if}
											</div>
		...
		{#if data.uncategorizedThreads && data.uncategorizedThreads.length > 0}
			<div class="bg-yellow-50 dark:bg-yellow-900/20 border border-yellow-300 dark:border-yellow-700 rounded-lg p-5 mb-6">
				<div class="flex items-start gap-3">
					<MessageSquare class="w-6 h-6 text-yellow-600 shrink-0 mt-0.5" />
					<div class="flex-1">
						<h3 class="font-semibold mb-3">Uncategorized Messages</h3>
						<div class="space-y-2">
		...
		{#if visibleNewClients.length === 0 && (!data.uncategorizedThreads || data.uncategorizedThreads.length === 0)}
			<div class="bg-background border border-border rounded-lg p-8 text-center">
				...
				<h3 class="font-semibold text-lg mb-1">All caught up!</h3>
				<p class="text-sm text-muted-foreground">No pending client registrations or uncategorized messages.</p>
			</div>
		{/if}
				...
				<table class="w-full">
					<thead class="bg-muted">
						<tr>
							...
							<th class="text-left px-4 py-3 text-sm font-semibold hidden sm:table-cell">Size</th>
							<th class="text-left px-4 py-3 text-sm font-semibold hidden sm:table-cell">Uploaded</th>
					...
					<tbody>
						{#each data.documents as doc}
							<tr class="border-t border-border hover:bg-muted/50 transition-colors">
								...
								<td class="px-4 py-3 text-sm text-muted-foreground hidden sm:table-cell">{(doc.fileSize / 1024).toFixed(1)} KB</td>
								<td class="px-4 py-3 text-sm text-muted-foreground hidden sm:table-cell">{formatDate(doc.uploadedAt)}</td>
				...
				<table class="w-full">
					...
					<tbody>
						{#each data.invoices as invoice}
							<tr class="border-t border-border hover:bg-muted/50 transition-colors">
								<td class="px-6 py-4 text-sm">{invoice.description}</td>
								<td class="px-6 py-4 font-semibold text-sm">{formatCurrency(invoice.amount)}</td>
								<td class="px-6 py-4 text-sm text-muted-foreground hidden sm:table-cell">{formatDate(invoice.dueDate)}</td>
								<td class="px-6 py-4">
