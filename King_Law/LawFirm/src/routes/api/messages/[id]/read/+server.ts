...
export const PATCH: RequestHandler = async ({ params, locals }) => {
	...
	// Get the case to verify access (handle nullable caseId)
	if (message.caseId) {
		...
		// Uncategorized message - verify recipient
		if (message.recipientId !== locals.user.id && locals.user.role !== 'admin') {
			throw error(403, 'Access denied');
		}
	}
