...
test.describe('Family Registration Journey', () => {
  ...
  test('complete family registration flow', async ({ page }) => {
    ...
    // Should redirect to event setup
    await expect(page).toHaveURL(/\/memorials\/[^\/]+\/setup/);
    await expect(page.getByText(/event for john doe/i)).toBeVisible();
  });
  ...
  test('shows loading state during registration', async ({ page }) => {
    ...
    // Check loading state
    await expect(page.getByText(/creating event/i)).toBeVisible();
  ...
  test('auto-generates event slug correctly', async ({ page }) => {
    await page.goto('/register/new-event-and-account');
  ...
  test('shows event URL preview', async ({ page }) => {
    await page.goto('/register/new-event-and-account');
  ...
  test('handles network errors gracefully', async ({ page }) => {
    ...
    await page.route('/api/register/new-event-and-account', route => {
      route.abort('failed');
    });
    ...
    await expect(page.getByRole('button', { name: /create event/i })).toBeEnabled();
  });
});
