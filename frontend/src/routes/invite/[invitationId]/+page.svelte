...
<div class="min-h-screen bg-gradient-to-br from-yellow-50 via-orange-50 to-amber-50 relative overflow-hidden">
	<!-- Animated background elements -->
	<div class="absolute inset-0 overflow-hidden pointer-events-none">
		<div class="absolute -top-40 -right-40 w-80 h-80 bg-gradient-to-br from-yellow-200/30 to-transparent rounded-full blur-3xl animate-pulse"></div>
		<div class="absolute -bottom-40 -left-40 w-96 h-96 bg-gradient-to-tr from-amber-200/20 to-transparent rounded-full blur-3xl animate-pulse delay-1000"></div>
	</div>

	<div class="relative z-10 max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
		<div class="text-center mb-12">
			<div class="w-24 h-24 mx-auto rounded-full bg-gradient-to-r from-yellow-600 to-amber-600 flex items-center justify-center mb-6 shadow-2xl">
		...
		{#if memorial}
			...
			<div class="bg-white/80 backdrop-blur-xl rounded-3xl shadow-2xl border border-white/20 p-8 mb-8">
				<div class="flex items-center space-x-6 mb-6">
					<div class="w-20 h-20 rounded-full bg-gradient-to-r from-yellow-600 to-amber-600 flex items-center justify-center shadow-lg">
				...
				<div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
					<div class="flex items-center space-x-3 p-4 rounded-2xl bg-gradient-to-r from-yellow-50 to-amber-50 border border-yellow-100">
						<Calendar class="w-6 h-6 text-yellow-600" />
						<div>
							<p class="text-sm font-medium text-yellow-600">Created</p>
							<p class="text-gray-900 font-semibold">
								{new Date(memorial.createdAt).toLocaleDateString()}
							</p>
						</div>
					</div>
					<div class="flex items-center space-x-3 p-4 rounded-2xl bg-gradient-to-r from-amber-50 to-orange-50 border border-amber-100">
						<Heart class="w-6 h-6 text-amber-600" />
						<div>
							<p class="text-sm font-medium text-amber-600">Role</p>
		...
		{:else if data.needsLogin}
			...
			<div class="bg-white/80 backdrop-blur-xl rounded-3xl shadow-2xl border border-white/20 p-8 text-center">
				<div class="w-16 h-16 mx-auto rounded-full bg-gradient-to-r from-yellow-500 to-amber-600 flex items-center justify-center mb-6 shadow-lg">
				...
				<p class="text-gray-600 mb-6">
					...
					<span class="font-semibold text-yellow-600">{invitation.inviteeEmail}</span>
				</p>
				<a 
					href="/login?redirect=/invite/{invitation.id}"
					class="inline-flex items-center px-8 py-4 bg-gradient-to-r from-yellow-500 to-amber-600 text-white rounded-2xl font-semibold text-lg hover:shadow-lg transition-all duration-300 hover:scale-105"
				>
		...
		{:else if data.emailMismatch}
			...
			<div class="bg-white/80 backdrop-blur-xl rounded-3xl shadow-2xl border border-white/20 p-8 text-center">
				...
				<p class="text-gray-600 mb-2">
					This invitation is for: <span class="font-semibold text-yellow-600">{invitation.inviteeEmail}</span>
				</p>
				<p class="text-gray-600 mb-6">
					You're signed in as: <span class="font-semibold text-amber-600">{user?.email}</span>
				</p>
				<div class="space-y-4">
					<a 
