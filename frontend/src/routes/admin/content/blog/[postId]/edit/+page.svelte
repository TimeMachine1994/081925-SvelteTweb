...
Edit existing blog post with all fields
...
<script lang="ts">
	...
	let featuredImage = $state(post.featuredImage || '');
	let status = $state(post.status);
	let metaTitle = $state(post.seo?.metaTitle || post.title);
	let metaDescription = $state(post.seo?.metaDescription || post.excerpt);
	let keywords = $state(post.seo?.keywords?.join(', ') || '');
	...
	async function handleSave() {
		...
		isProcessing = true;
		processingMessage = 'Saving changes...';
			...
			<div class="form-section">
				...
				<div class="form-group">
					<label for="slug">
						...
						<span class="hint">Used in the post URL</span>
					</label>
