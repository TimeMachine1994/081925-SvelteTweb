...
export const POST: RequestHandler = async ({ request, locals, getClientAddress }) => {
	...
	try {
		...
		console.log('🚫 [ADMIN API] Rejecting funeral director:', directorId);
		...
		await logAuditEvent({
			uid: locals.user.uid,
			action: 'funeral_director_rejected',
		...
		console.log('✅ [ADMIN API] Funeral director rejected successfully');

		return json({
			success: true,
			message: 'Funeral director rejected successfully'
		});

	} catch (error) {
		console.error('❌ [ADMIN API] Error rejecting funeral director:', error);

		// Log the error
		await logAuditEvent({
			uid: locals.user?.uid || 'unknown',
			action: 'funeral_director_rejection_error',
			userEmail: locals.user?.email || 'unknown',
			userRole: (locals.user?.role as 'admin' | 'owner' | 'funeral_director') || 'admin',
			resourceType: 'funeral_director',
			resourceId: 'rejection_error',
		...
		return json(
			{
				error: 'Failed to reject funeral director',
