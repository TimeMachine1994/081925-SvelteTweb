...
export const POST: RequestHandler = async ({ request, cookies }) => {
	try {
		...
		if (!hasPermission) {
			console.log(
				'🔒 [DRAFT API] Insufficient permissions for user:',
				userId,
				'role:',
				decodedClaims.role
			);
			console.log(
				'🔒 [DRAFT API] Memorial owner:',
				memorialData?.ownerUid,
				'funeral director:',
				memorialData?.funeralDirectorUid
			);
		...
		await saveSlideshowDraft(draftData);
...
export const GET: RequestHandler = async ({ url, cookies }) => {
	try {
		...
		if (!hasPermission) {
			console.log(
				'🔒 [DRAFT API GET] Insufficient permissions for user:',
				userId,
				'role:',
				decodedClaims.role
			);
			console.log(
				'🔒 [DRAFT API GET] Memorial owner:',
				memorialData?.ownerUid,
				'funeral director:',
				memorialData?.funeralDirectorUid
			);
		...
		const draftData = await getSlideshowDraft(memorialId, userId);
...
export const DELETE: RequestHandler = async ({ url, cookies }) => {
	try {
		...
		if (!hasPermission) {
			console.log(
				'🔒 [DRAFT API DELETE] Insufficient permissions for user:',
				userId,
				'role:',
				decodedClaims.role
			);
		...
		await deleteSlideshowDraft(memorialId, userId);
