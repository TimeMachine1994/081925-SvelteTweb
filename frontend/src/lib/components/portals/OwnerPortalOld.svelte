...
<div class="max-w-6xl mx-auto px-4 py-6">
	...
	{#if memorials && memorials.length > 0}
		...
		{#if currentMemorial}
			...
			{:else if currentPaymentStatus === 'saved_pending_payment'}
				<div class="bg-blue-100 border-l-4 border-blue-500 text-blue-700 p-4 mb-6" role="alert">
					<p class="font-bold">Package Saved!</p>
					<p>Your livestream package details have been saved. Please complete the payment to finalize your booking.</p>
				</div>
			{/if}
			...
			<!-- Livestream Schedule Table (always show if config exists) -->
			{#if currentMemorial.livestreamConfig}
				<LivestreamScheduleTable memorial={currentMemorial} />
			{:else}
				<div class="bg-gray-50 border border-gray-200 text-gray-700 p-4 mb-6 rounded-lg">
					<p class="font-bold">No Livestream Package Selected</p>
					<p>It looks like you haven't selected a livestream package for this memorial yet. Please visit the <a href="/app/calculator/?memorialId={currentMemorial.id}" class="text-purple-600 hover:underline">calculator page</a> to choose your options.</p>
				</div>
			{/if}
			...
			<!-- Bottom Pay Now Button (show if payment is incomplete or saved_pending_payment) -->
			{#if currentPaymentStatus === 'incomplete' || currentPaymentStatus === 'saved_pending_payment'}
