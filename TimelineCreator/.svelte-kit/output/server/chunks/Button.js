...
function Button($$renderer, $$props) {
  ...
  $$renderer.push(`<button${attr("type", type)}${attr_class(`${stringify(baseClasses)} ${stringify(variantClasses[variant])} ${stringify(sizeClasses[size])}`)}${attr("disabled", disabled || loading, true)}>`);
  if (loading) {
    $$renderer.push("<!--[-->");
